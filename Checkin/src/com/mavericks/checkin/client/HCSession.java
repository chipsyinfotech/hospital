...
public class HCSession {
	...
	public void storeSession(Context c,HCProfileHolder profile) {
		if(profile == null)
			return;
		HCUtils.editPrefString(HCConstants.PREF_ABRNID, profile.getAbaranId(), c);
		HCUtils.editPrefString(HCConstants.PREF_FBID, profile.getFbId(), c);
		...
		HCUtils.editPrefString(HCConstants.PREF_TWID, profile.getTwId(), c);
	...
	public String getAbharanId(Context c) {
		return 	HCUtils.getPrefString(HCConstants.PREF_ABRNID, "", c);
	}
	...
	public void clearSession(Context c) {
		HCUtils.removePreference(HCConstants.PREF_ABRNID,  c);
