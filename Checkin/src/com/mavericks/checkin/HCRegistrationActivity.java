...
public class HCRegistrationActivity extends HCBaseActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSpnspecial = (Spinner) findViewById(R.id.spinner_special);
		...
		mTxtdoctor = (TextView) findViewById(R.id.txt_doctor);
	...
	private void newvisit() {
		...
		formData.add(new BasicNameValuePair(HCConstants.PAR_HOSPITAL_ID,hospital_id));
		formData.add(new BasicNameValuePair(HCConstants.PAR_SPECIAL_ID,special_id));
	...
	private void revisit() {
		...
		formData.add(new BasicNameValuePair(HCConstants.PAR_DNAME, ""
				+ mTxtdoctor.getText().toString()));
		HCClient.getInstance().request(this, HCServerUtils.REQ_HOSPITAL_REVISIT,
				null,formData,null,parser, new HCIRequestListener() {
					@Override
					public void onComplete(int req_type, int status) {
						hideProgressDialog();
